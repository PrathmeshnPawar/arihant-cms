...
export default function TagsPage() {
  ...
  const [form, setForm] = React.useState<TagForm>(emptyForm);
  ...
  async function fetchTags() {
    ...
    try {
      ...
      if (!res.ok) {
        toast("error", json?.message || "Failed to fetch tags");
        setRows([]);
        return;
      }
  ...
  async function submitTag(values: TagForm) {
    setSaving(true);
    try {
      const url =
        dialogMode === "create"
      ...
      toast("success", dialogMode === "create" ? "Tag created" : "Tag updated");
      setDialogOpen(false);
      await fetchTags();
    } catch (e: any) {
      toast("error", e?.message || "Network error");
  ...
  async function doDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/tags/${deleteTarget._id}`, {
        method: "DELETE",
        credentials: "include",
      });
    ...
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
      <Box>
        ...
        <Typography variant="body2" color="text.secondary">
          Manage keyword tags for posts (search & filtering)
        </Typography>
      </Box>
      ...
      <Paper sx={{ borderRadius: 3, overflow: "hidden" }}>
        ...
        >
          <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
            ...
            <IconButton onClick={fetchTags} title="Refresh">
              <RefreshIcon />
            </IconButton>
          </Box>
