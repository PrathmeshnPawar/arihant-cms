...
import { getTagPost } from "../../../../lib/blogServerUtils/TagPage";

export default async function TagArchivePage({
  params,
}: {
  params: { slug: string }; // ✅ Must match [slug] folder name
}) {
  const { slug } = params;

  const { posts, tagDoc } = await getTagPost(slug);
  if (!tagDoc) {
    return (
      <Container sx={{ py: 10, textAlign: "center" }}>
        ...
        <Link
          href="/blog"
          style={{ marginTop: "20px", display: "block", color: "#1976d2" }}
        >
          Back to Blog
        </Link>
      </Container>
